...
**Pendiente: rehacer `DeliverableSummary` — feedback de Jorge (2026-07-23), primera versión no convenció.** La primera versión (`summary` por paquete, caja "Qué vas a recibir" justo después de los datos del cliente) tiene varios problemas reales que Jorge señaló:
- **Tono equivocado:** usaba voseo ("tu marca", "subís productos", "manejás fotos vos mismo"). Al ser una cotización formal, el registro correcto es **usted**, no vos — inconsistente con el resto del documento que ya habla en tercera persona neutra ("el cliente").
- **Nombra tiers internos que el cliente no conoce:** el resumen de Pro literalmente dice "Todo lo del paquete Negocio, más..." — el cliente no necesariamente sabe qué es "Vitrina" o "Negocio", son nombres internos de Jodabor, no conceptos que el cliente reconozca. Esto es justo el tipo de jerga que se supone que esta caja debía eliminar.
- **Debe ir al final, no al inicio:** Jorge quiere el resumen **después** de toda la lista técnica de features, no antes — como un cierre tipo "Entregable final: ..." que amarra todo lo que ya se detalló arriba, no como introducción.
- **Debe ser específico/cabal, no genérico:** en vez de una frase corta tipo marketing, Jorge quiere algo más descriptivo y concreto: qué secciones tiene la página, qué flujo sigue, cómo funciona, qué características tiene, y que el diseño es personalizado para ese cliente — ejemplo de tono que dio: "Entregable final: una página web nueva con secciones tales, con siguiente flujo, que funciona de esta manera y tiene estas características, según diseño personalizado para el cliente..."
- **Debe aplicar también a selección a la carte, no solo a los 3 paquetes fijos.** Hoy `DeliverableSummary` solo funciona con `activePackage.summary` (string estático por paquete) y no renderiza nada si el cliente arma su cotización a la carte. La versión final necesita generar este párrafo de cierre dinámicamente a partir de los `visibleBlocks`/features realmente seleccionados (paquete o a la carte), no depender de un texto fijo por tier.

Jorge pidió guardar esto en memoria y trabajarlo en una sesión posterior — no rehacer todavía. Mientras tanto, la caja actual sigue en `main` pero con el bug conocido de nombrar "paquete Negocio" en el resumen de Pro; ese texto específico no debería usarse tal cual en una cotización real hasta corregirse.
