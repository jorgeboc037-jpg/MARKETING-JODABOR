...
**Resuelto (2026-07-23): `DeliverableSummary` rehecho según el feedback de Jorge.** La primera versión (`summary` estático por paquete, caja "Qué vas a recibir" justo después de los datos del cliente) tenía voseo, nombraba tiers internos ("todo lo del paquete Negocio") y no aplicaba a la carte. Se reemplazó por un párrafo **generado dinámicamente**:

- Cada feature en `src/data/features.js` tiene ahora un campo `phrase` (fragmento corto en lenguaje llano, tono neutro tercera persona — "el cliente"/"el negocio", igual que `LegalNotes`/`SignatureBlock`). `setup_base` y `supabase_setup` (plomería técnica invisible) no llevan `phrase` y quedan excluidos de la narrativa.
- `src/pdf/deliverySummary.js` (función pura `buildEntregableFinal`) arma el párrafo agrupando por bloque (`paginas` → secciones, `funcionales` → cómo funciona, `admin` → qué se puede gestionar, `extras` → qué suma, `customItems` → cargos adicionales) y cierra siempre con "Todo esto según diseño personalizado para el negocio del cliente."
- `DeliverableSummary.jsx` ya no recibe `activePackage`, recibe `visibleBlocks` + `customItems` — funciona igual para los 3 paquetes fijos y para selección a la carte.
- Se movió el bloque **al final** de la lista de features (después de "Cargos adicionales", antes del subtotal), como pidió Jorge, en vez de ir justo después de los datos del cliente.
